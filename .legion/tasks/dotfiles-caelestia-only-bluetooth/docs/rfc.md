...
> **Profile**：RFC Heavy
> **Status**：Ready for re-review（Revision 5）
> **日期**：2026-07-12
...
- **rfkill**：普通主机Revision 4 ExecStopPost不变。TLP host的generic helper不再WantedBy multi-user；由`tlp.service Wants=`拉起并`After=tlp.service`完成boot finalization，ADD使用独立`@%k`instance，resume恢复post-resume trigger。
...
Revision 4的ordinary finalizer与全部Agent1/privacy/Caelestia/package边界已经评审通过。Post-implementation review发现TLP production graph中`multi-user Wanted helper`+`helper After TLP`形成真实cycle，且实现的per-device template/`tlp-sleep`hook偏离已评审TLP设计。Revision 5只重审TLP boot/add/resume wiring。
...
`${bluetoothRfkillFinalize}`同时供ordinary ExecStopPost与TLP/ordinary base、TLP add template使用；它没有poll、sleep、systemctl或event identity：
...
#### 5.8.3 shared projection与ordinary保持不变

共享`mkBluetoothRfkillProjection`只按`config.services.tlp.enable`分支，不读取hostname。概念形状：

```nix
baseHelper = helperService // {
  wantedBy = lib.optionals (!tlpEnabled) [ "multi-user.target" ];
  after = lib.optionals tlpEnabled [ "tlp.service" ];
};

tlpBootWants = lib.optionals tlpEnabled [
  "bluetooth-rfkill-unblock.service"
];

eventTemplate = helperService // {
  after = lib.optionals tlpEnabled [ "tlp.service" ];
};
```

ordinary host行为不变：base helper仍`WantedBy=multi-user.target`；udev Bluetooth ADD仍target `systemd-rfkill.service`；resume仍由`post-resume.service`nonblocking restart base helper。Revision 4 ExecStopPost及其fixture不改。

#### 5.8.4 TLP boot：declarative post-start trigger

TLP branch对base helper设：

- `WantedBy=[]`，不得出现在`multi-user.target.wants/`；
- `After=tlp.service`；
- `ExecStart`仍是无参数immutable finalizer。

共享profile同时给`tlp.service`增加弱`Wants=bluetooth-rfkill-unblock.service`。这是选定的“post-start trigger”：Wants把helper拉入TLP start transaction，After保证pinned TLP `Type=oneshot`的`tlp init start`成功完成、service fully started后helper才开始。成功boot ordering只有：

```text
multi-user.target → tlp.service → bluetooth-rfkill-unblock.service
```

vendor TLP保留`WantedBy=multi-user.target`与`After=multi-user.target NetworkManager.service`。因为multi-user不再直接Wants helper，不会再生成`multi-user After helper`回边。不得用Ramen host override。

采用weak Wants而非Requires：helper enqueue/start失败不得把TLP服务反向停止或改变其power policy；release gate分别要求TLP成功与boot helper取得成功InvocationID。boot不从TLP ExecStartPost同步调用systemctl；`ExecStartPost=systemctl --no-block`虽可enqueue后返回，但比纯dependency多一层manager CLI/failure path，不采用。直接在TLP ExecStartPost运行finalizer也因failure coupling不采用。

#### 5.8.5 TLP add、并发与resume

TLP udev rule为：

```udev
ACTION=="add", SUBSYSTEM=="rfkill", ATTR{type}=="bluetooth", TAG+="systemd", ENV{SYSTEMD_WANTS}+="bluetooth-rfkill-unblock@%k.service"
```

只在TLP branch生成`bluetooth-rfkill-unblock@.service`。每个kernel rfkill name得到独立unit；base boot/resume helper activating时，ADD template job不会与它合并。template也`After=tlp.service`，但不Wants任何target或masked unit。

`%k`/`%I`只能决定unit identity和可选Description。最终template `ExecStart`必须逐字等于`${bluetoothRfkillFinalize}`，不含`%i`/`%I`、参数、instance-derived环境或shell expansion；finalizer仍扫描全部Bluetooth entries，因此instance绝不成为root command input。

resume恢复Revision 4路径：NixOS `post-resume.service`中的`powerManagement.resumeCommands`执行：

```sh
systemctl --no-block restart bluetooth-rfkill-unblock.service
```

Pinned `tlp-sleep.service ExecStop=tlp resume`/`restore_device_states`先完成，既有post-resume ordering随后enqueue新base InvocationID。`restart`避免与任何尚未结束的base invocation合并；`--no-block`只验证/enqueue，不在post-resume service中同步等待。删除当前实现的`tlp-sleep.postStop`；不得添加`After/PartOf=tlp-sleep.service`。

若ADD发生在resume期间：早到的template不替代post-resume base finalization；晚到的ADD又有独立template。因此TLP restore后始终至少有一个不被ADD合并的finalizer。

TLP branch继续没有systemd-rfkill ExecStopPost/drop-in，stock `systemd-rfkill.service/socket`保持NixOS TLP module的disabled/masked状态。base/template/TLP Wants/udev/post-resume均不得引用、start或unmask它们。finalizer只写Bluetooth，WLAN live/persisted与TLP配置/state不变。

#### 5.8.6 TLP failure与observability

- TLP boot必须分别出现成功`tlp.service`与后续base helper InvocationID；任一缺失/failed都阻塞，但weak Wants不改变TLP failure propagation；
- full-target verify即使exit 0，只要输出`ordering cycle`、`Found ... cycle`或`Job ... deleted to break ordering cycle`就失败；PID 1 boot journal同样扫描；
- no-device/already-unblocked是成功零写；base/template failure只归属各自unit；
- template与base并发时必须有两个不同unit/job/InvocationID；
- 不使用event polling或自定义等待窗口；只执行一次同步Bluetooth-only finalizer。
...
| OnSuccess/外部watcher | 不选：TLP RemainAfterExit不会在boot进入OnSuccess终态，watcher又增加daemon |
| TLP helper `WantedBy=multi-user` + `After=tlp` | 拒绝：production full-target graph已证明cycle并删除job |
| TLP direct/CLI ExecStartPost | 不选：direct扩大TLP failure；`systemctl --no-block`虽可行但比declarative dependency复杂 |
| `tlp-sleep.postStop` resume hook | 拒绝：偏离已评审post-resume路径且不必要 |
| **ordinary per-invocation ExecStopPost；TLP `tlp Wants base` + base/template After TLP + post-resume** | 采用 |

### 6.3 Decision

采用：**私有 Blueman runtime + 单command安全runner/local GTK pairing + 全局surface删除 + ordinary per-invocation rfkill finalizer/TLP cycle-free base+template helpers + 同源可测试Caelestia JS policy**。有意放弃Bluetooth soft-block跨boot/resume保留语义；不放弃WLAN/hard-block/TLP语义，也不改变Caelestia全局notification persistence。
...
3. Ramen：先验证full-target无cycle、TLP/mask与WLAN状态不变，再测boot InvocationID、per-device concurrent ADD和post-resume helper。
4. 其余主机逐台部署；至少验证 unit、surface、soft block。

### 7.3 Rollback

1. Caelestia policy 异常：只回退 package override/policy。
2. rfkill finalizer异常：ordinary按Revision 4回滚；TLP移除`tlp.service Wants=base`、template/udev与post-resume trigger，但不得恢复成multi-user WantedBy或tlp-sleep hook；仍不恢复旧block，必要时手工`rfkill unblock bluetooth`。
...
- system journal：ordinary systemd-rfkill/ExecStopPost结果；TLP init/resume completion、base/template InvocationID与finalizer result；不记录设备路径、instance-derived命令参数或WLAN值。
...
- root 仅运行固定参数 rfkill oneshot；TLP template instance只选择unit identity，绝不进入ExecStart/环境/参数；没有Blueman mechanism、polkit action或用户输入。
...
| ExecStopPost/base/template helper后Bluetooth仍soft=1 | finalizer non-zero、所属unit失败 | 阻塞 |
| no-device/already-unblocked | finalizer零写成功；未来device add由udev另行触发 | 非故障；当前进程不等待且不得制造socket loop |
| TLP full-target cycle/job-deleted-to-break-cycle | boot helper可能不执行 | 阻塞；verify status 0也不得忽略诊断文本 |
| TLP boot无helper InvocationID | 不以ADD补救伪装成功 | 阻塞；检查TLP Wants/After与boot journal |
| TLP add与base helper job合并 | 该ADD无独立template evidence | 阻塞；template unit/InvocationID必须不同 |
...
每个存在Bluetooth entry的invocation都在ExecStopPost完成点断言soft=0（finalizer失败case除外并明确failed）；no-device case断言零写；所有case中WLAN live value、persisted bytes/hash及fake write log逐位证明未被finalizer触碰。本fixture的ordinary场景与Revision 4完全相同；TLP由下一fixture验证。

#### TLP production graph fixture

TLP VM必须使用同一production projection factory，并保持pinned vendor unit graph：从`pkgs.tlp`派生test package时只替换`sbin/tlp`行为，不能用无vendor units的shell package后手写`tlp.service`。静态断言生成的TLP unit仍含`After=multi-user.target NetworkManager.service`、`Type=oneshot`、`RemainAfterExit=yes`与multi-user enablement；fixture init dependency只能append。

VM从默认boot启动完整`multi-user.target` transaction，boot helper不得由test script手工start。必须同时证明：

1. generated `multi-user.target.wants/`没有base helper；`tlp.service Wants=base`；base/template均`After=tlp.service`；
2. pinned `systemd-analyze verify multi-user.target`输出和exit status都被检查；任何`ordering cycle`、`Found ... cycle`或`Job ... deleted to break ordering cycle`文本失败，即使status=0；PID 1 boot journal也无同类诊断；
3. fake TLP `init-end` timestamp严格早于base helper第一条finalizer journal；base有真实InvocationID且Bluetooth soft=0；
4. generated udev精确target `bluetooth-rfkill-unblock@%k.service`；template `ExecStart`是无参数finalizer且不含`%i/%I`；
5. 真实`hci_vhci`/udev ADD产生template InvocationID；hold base helper并再ADD时，PID 1同时列出base和template两个job，unit名与InvocationID均不同；释放后全部Bluetooth soft=0；
6. 实际NixOS post-resume transaction执行fake `tlp resume`后才运行production `resumeCommands`并得到新base InvocationID；test不得直接start helper；可替换实际sleep syscall/executor，但不得改sleep/post-resume/TLP ordering edges；generated `tlp-sleep.service`没有Bluetooth postStop/drop-in；
7. helper command failure可见且下一invocation可恢复；stock systemd-rfkill始终masked/inactive/无InvocationID；WLAN live/persisted与TLP state逐字节及hash不变。

同一gate还要从实际Ramen专项eval生成relevant production unit tree（multi-user target/wants、pinned TLP units、base/template、post-resume）并运行full-target verify，不能只verify单个文件；不得因此请求Ramen完整toplevel或无关package closure。fixture和host projection任一不一致即失败。
...
| rfkill path | 四台ordinary host保持Revision 4；Ramen stock units masked/无drop-in，base无WantedBy且After TLP，TLP Wants base，udev targets `@%k`template，resume在post-resume，tlp-sleep无Bluetooth hook |
| TLP graph | Ramen full `multi-user.target` verify无cycle/job-deleted-to-break-cycle诊断；vendor `After=multi-user.target NetworkManager.service`仍在；boot helper取得独立InvocationID |
...
- shared rfkill finalizer、ordinary per-invocation/socket-reactivation fixture、faithful TLP full-target VM与ordinary/TLP两种生成unit；
- 实际Ramen专项eval导出的relevant production unit tree及其`systemd-analyze verify multi-user.target`诊断扫描；
...
2. 部署前后`services.tlp.enable=true`；`systemd-rfkill.service/socket`仍disabled/masked且没有ExecStopPost drop-in或InvocationID。
3. generated graph必须显示base helper无multi-user WantedBy、`After=tlp.service`，`tlp.service Wants=base`且保留vendor `After=multi-user.target NetworkManager.service`；full-target verify/journal无cycle或job-deleted-to-break-cycle文本。
4. reboot后journal严格显示TLP init完成，再出现新的base helper InvocationID与Bluetooth soft=0；boot helper不得靠device ADD偶然补救。
5. 记录WLAN soft/hard、persisted state与TLP配置/state；boot/add/resume前后逐位相同。
6. suspend/resume journal必须显示`tlp resume`/`restore_device_states`完成，随后`post-resume.service`enqueue新的base helper InvocationID；`tlp-sleep`无Bluetooth hook。
7. tagged ADD拉起`@rfkillN`template；在base helper activating窗口再ADD时两个job/InvocationID并存，最终全部Bluetooth soft=0，hard block不被伪装。
...
- ordinary Revision 4 finalizer回归；或Ramen出现full-target cycle/job-deleted-to-break-cycle、boot helper无InvocationID、ADD与base job合并、tlp-sleep Bluetooth hook、masked stock activation、WLAN/TLP状态变化；
...
- 删除Blueman/Rofi/Hyprland surface；ordinary Revision 4 ExecStopPost；TLP declarative boot base、per-device add template与post-resume trigger；host-conditional tagged udev。
- 验收：ordinary socket-reactivation fixture、TLP faithful full-target/cycle/concurrent-add fixture与WLAN-sentinel gate通过；五主机专项eval及Axiom/Ramen unit derivation通过。
...
- TLP vendor `After=multi-user.target NetworkManager.service`与NixOS post-resume graph均为pinned inputs；TLP/systemd/NixOS更新时必须重跑full-target diagnostics和boot/add/resume InvocationID gates。
...
| Revision 4 TLP决策 | post-implementation状态 | Revision 5决定 |
|---|---|---|
| generic helper `WantedBy=multi-user.target`、不依赖TLP | 与实现新增的`After=tlp.service`组合成production cycle | TLP generic移除WantedBy；改由`tlp.service Wants=generic`并保留`generic After=tlp`，形成declarative post-start trigger |
| ADD target generic helper | 实现改成`@%k`template，未经RFC但真实解决与boot/resume base合并 | 正式采用per-device template；instance只影响unit identity，ExecStart仍immutable |
| resume由`post-resume.service`触发generic helper | 实现偏移到`tlp-sleep.postStop` | 恢复post-resume nonblocking restart，删除全部tlp-sleep Bluetooth hook |
| stock systemd-rfkill保持masked；Bluetooth-only/WLAN不变 | 实现与测试保持 | 不变，并加入mask/InvocationID及WLAN/TLP byte/hash gate |
| shared Bluetooth profile按TLP capability分支 | 风险若以Ramen override修补 | 明确禁止host override，未来TLP Bluetooth host自动获得相同graph |
...
| Revision 2/3剩余：ordinary rfkill final writer | 5.8.1–5.8.3、9、10.1–10.4 |
| Revision 5：TLP production cycle/implementation deviation | 5.8.3–5.8.6、6.2、9、10.1–10.4、Revision 4映射表 |
| Revision 2剩余：pairing history privacy | 5.6.2、8、9、10.1、10.4 |

Revision 5只替换TLP wiring及其fixture fidelity；Revision 4 ordinary finalizer与Agent1/privacy/Caelestia/wrapper/host-scope决策保持不变。cycle-free graph、per-device template、post-resume路径和full-target negative gate均已定义，状态为 **Ready for re-review**。尚未授权生产修改。

## 15. References

- Contract：`../plan.md`
- Research：`research.md`
- RFC reviews：`review-rfc.md`
