...
`aliyun-acorn` 位于阿里云 ECS 环境，官方 Nix cache 和海外 Cachix 在国内网络下可能较慢。仓库全局配置已经声明 nix-community 与 Hyprland Cachix；NixOS 会自动补充 `https://cache.nixos.org/`。本任务需要在不影响其他 hosts 的情况下，让 `aliyun-acorn` 优先尝试国内 mirror。

## 方案

在 `hosts/aliyun-acorn/default.nix` 的 host config 中添加：
...
`lib.mkBefore` 会把 TUNA mirror 放在已有全局 substituters 前面，同时保留仓库现有 Cachix 和 NixOS 默认官方 cache fallback。
...
删除 `hosts/aliyun-acorn/default.nix` 中新增的 `nix.settings.substituters` 块，或 revert 本 PR。回滚不会迁移数据，也不会影响其他 host。
