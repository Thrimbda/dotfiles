...
Live Cloudflare control-plane work is blocked: the available encrypted token can read DNS but receives `403` from Zero Trust Access endpoints, so no Access app/policy or DNS route was created.
