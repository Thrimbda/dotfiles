...
The reverse tunnel shape remains remote loopback `127.0.0.1:2223` to Axiom local `127.0.0.1:22`. Validation passed for generated service and healthcheck shape, Axiom toplevel build, service-style `c1` SSH authentication, and a temporary reverse endpoint identity smoke test.

## Reusable Decisions

- Axiom's autossh tunnel to `8.159.128.125` uses remote account `c1`.
- Axiom's autossh service and endpoint-key healthcheck should not depend on mutable user known-hosts state; they use `UserKnownHostsFile=/dev/null` with a service-specific generated known-hosts file.
