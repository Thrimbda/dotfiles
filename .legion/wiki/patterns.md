...
For Axiom Keep Awake changes, prefer Caelestia's native `idleInhibitor` over custom no-sleep wrappers when the requirement is graphical-session idle inhibition. Hypridle should remain scoped to idle lock and DPMS for the current Axiom default: validate the checked-in config has the 15 minute lock listener, the 30 minute DPMS off/on listener, and no `$suspend_cmd`, `systemctl suspend`, `loginctl suspend`, or automatic suspend listener. Validate the `07-caelestia-keep-awake` startup hook runs after the session-owned `06-caelestia-shell` runner, absence of stale `axiom-sleep-mode` packages/launchers/direct Hypridle overrides/sleep-inhibitor services, and README documentation of the graphical-session boundary. Also validate generated helper text: Caelestia IPC helpers should call the evaluated `caelestia-shell` binary path or otherwise own command lookup, retry long enough for cold-start IPC registration, and launch asynchronously so desktop startup is not blocked. If a future task restores stronger default never-sleep behavior, require a new scoped contract and validation plan rather than reviving historical inhibitor wiring implicitly. Avoid triggering live suspend or hibernate in tool sessions; record those as post-deploy Hyprland smoke checks.
