...
For Axiom Keep Awake changes, prefer Caelestia's native `idleInhibitor` over custom no-sleep wrappers when the requirement is graphical-session idle inhibition. Validate the `07-caelestia-keep-awake` startup hook runs after the session-owned `06-caelestia-shell` runner, absence of stale `axiom-sleep-mode` services/packages/launchers/direct Hypridle overrides, and README documentation of the graphical-session boundary. Also validate the generated helper text: if the helper uses Caelestia IPC, prefer the evaluated `caelestia-shell` binary path or explicit session runner PATH ownership rather than a Python CLI path that later resolves `caelestia-shell` from ambient `PATH`; because `caelestia-session start` backgrounds the runner, the helper must retry long enough for cold-start IPC registration. Avoid triggering live suspend or hibernate in tool sessions; record those as post-deploy Hyprland smoke checks.
