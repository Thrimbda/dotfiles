...
For a machine-specific domestic Nix cache preference, add mirrors at the host level with `nix.settings.substituters = lib.mkBefore [ "<mirror>" ... ];` rather than changing global flake inputs or forcing all hosts to use the same mirrors. Keep existing Cachix and `https://cache.nixos.org/` fallback unless the task explicitly scopes an offline or official-cache-blocked environment. For the China-hosted Alibaba Cloud `acorn` machine, the current order is TUNA, USTC, then SJTU.
...
For shared Playwright/dev modules that are imported by both NixOS and nix-darwin, guard NixOS-only options by option existence before generating the attrset. Use an `options` probe plus `optionalAttrs` for `programs.nix-ld` so Darwin never sees the unknown option; do not rely on `mkIf` around a platform condition for an option that may not exist. Validate the Darwin side with `darwin-rebuild build --flake .#charlie` and an explicit `nix build .#darwinConfigurations.charlie.system`, then validate the Linux intent with a focused `nix eval` of `nixosConfigurations.axiom.config.programs.nix-ld.libraries`.
...
When splitting ordered Nix lists such as `user.packages` or `environment.systemPackages` across imported modules, compare the evaluated order rather than only set membership. If a host-local package recipe must occupy a specific position, keep it in an adjacent private helper and insert the package from the module that owns the ordered list; do not add a public option solely to control merge order.
...
## 1Ex NAV-Only Fund Repair

For a personal portfolio Fund that exists only to display total-assets NAV, keep it private with `subscription_open=false` and do not create an owner investor, initial cash flow, or shares merely to make unit price move. Zero shares intentionally project unit price as `1`; the viewer metric is total-assets NAV.
...
For Axiom startup regressions where Caelestia is not running, test `hey hook startup` before changing Caelestia itself. A Janet native module error such as `config mismatch - host <new> vs. module <old>` means the managed JPM tree was compiled for an older Janet ABI; a missing module such as `spork/path` after activation usually means a network-backed `jpm deps` rebuild failed after active artifacts were removed. Fix the activation rebuild path with staging plus a `hey` smoke, then verify `hyprland-session.target` plus `caelestia-session` rather than adding a second shell launcher. The staged JPM rebuild environment must create `$JANET_TREE/lib/.cache`, provide `jpm`/`git`/compiler/coreutils on `PATH`, and set XDG fallbacks including `XDG_RUNTIME_DIR`; otherwise JPM can fetch partially but fail during git cache creation, nested `jpm deps`, or `hey` module loading.
...
For autossh reverse tunnel regressions, validate both sides of the generated shape: the remote-forward string must remain loopback-only and port-unique, and the local target service must exist as an active daemon if the tunnel forwards to `127.0.0.1:22`. Keep remote server host-key pinning separate from reverse endpoint identity. For mutable/reinstallable remotes such as Axiom's `8.159.128.125`, only use Nix-managed host-key pins when the task explicitly accepts the redeploy refresh burden; service-owned SSH paths should avoid mutable user known-hosts state if stale entries can block system services. Prefer `c1ctl autossh check` for Axiom's manual endpoint identity diagnostic instead of reintroducing a timer-backed systemd healthcheck.

For NixOS OpenSSH configuration, avoid using `services.openssh.extraConfig = lib.mkForce ""` to clear one inherited line. NixOS emits important generated `sshd_config` directives through `extraConfig`, including `AuthorizedKeysFile %h/.ssh/authorized_keys /etc/ssh/authorized_keys.d/%u`; forcing it empty can make sshd ignore generated `/etc/ssh/authorized_keys.d/<user>` files. Prefer removing the unsupported inherited line at its source, then validate the generated `sshd_config` and the generated `environment.etc."ssh/authorized_keys.d/<user>"` entry.
...
For nginx `auth_request` gateways that set host-only cookies and validate returns against a single public base URL, deploy one gateway instance per protected public origin unless upstream explicitly supports multi-origin return targets and cookie domain configuration. Validate both the Nix attrsets and the generated nginx config, because internal instance names can accidentally become `server_name` values if vhosts are keyed incorrectly.
...
For directly installed macOS GUI apps that ship CLI shims inside the `.app` bundle, do not enable an unrelated Nix module just to make the command visible. Add the app shim directory to the relevant shell init path, keep the installation source unchanged, and validate the generated shell env. For charlie's direct Emacs.app install, Doom CLI needs both `$XDG_CONFIG_HOME/emacs/bin` for `doom` and `/Applications/Emacs.app/Contents/MacOS/bin` for the `emacs` shim.

For Axiom Keep Awake changes, prefer Caelestia's native `idleInhibitor` over custom no-sleep wrappers when the requirement is manual graphical-session idle inhibition. Caelestia is the current Axiom idle owner: validate its generated and persisted `general.idle.timeouts` contain the 900 second `lock` and 1800 second `dpms off` / `dpms on` entries with no 600 second sleep/suspend/hibernate action, and validate `hypridle.service` is absent for Axiom while it remains present for a default-enabled host. Changing only seed settings is insufficient for an already-existing mutable `~/.config/caelestia/shell.json`. Validate there is no Axiom startup hook or helper that runs `idleInhibitor enable` by default, absence of stale `axiom-sleep-mode` packages/launchers/direct Hypridle overrides/sleep-inhibitor services, and README documentation of the graphical-session boundary. If a future task restores stronger default never-sleep behavior, default Keep Awake, or a second automatic idle owner, require a new scoped contract and validation plan rather than reviving historical wiring implicitly. Avoid triggering live suspend or hibernate in tool sessions; record long-idle DPMS/wake behavior as post-deploy Hyprland smoke evidence.
...
For the Axiom lock-scoped DPMS delay, arm from `WlSessionLock.secureChanged` rather than `lockedChanged`, a lock command, or a global last-input deadline. Quickshell 0.3 does not reliably notify `lockedChanged` when it creates a lock, while `secureChanged` follows compositor confirmation. Add a typed setting with a default of `0`, enable it only in both Axiom config paths, and make every lock epoch own a new timer. On unlock, invalidate and destroy the timer; timer expiry must check lock state, immutable epoch, and active-timer identity. Preserve the separate 900/1800-second policy and never add a second idle daemon, suspend, or hibernate.

For Axiom DPMS wake, let Hyprland own physical input through `misc.key_press_enables_dpms` and `misc.mouse_move_enables_dpms`. The latter is pointer motion, not a mouse-button click. This host-wide policy wakes the lock-scoped and 1800-second fallback DPMS paths without releasing `WlSessionLock`. Validate all four runtime transitions: locked DPMS-off, wake with lock retained, no rearm in the same epoch, and unlock cleanup after timer-owned DPMS-off.

The Config property and QML timer are built from different Caelestia source filesets. A main shell `overrideAttrs` does not automatically patch or rebuild its separately built `Caelestia.Config` plugin. Split the C++ Config and QML patches, apply each to its own derivation, capture the unmodified plugin before either override, and replace the exact shell `buildInputs` plugin only when its resolved store path equals the original plugin path. Assert exactly one match rather than matching names or appending another plugin, and extend `old.passthru` with `plugin = patchedPlugin` so the public plugin and shell dependency agree. Verify clean zero-fuzz patch application, the built plugin `qmltypes`, exact build-input replacement, `passthru.plugin`, and closure references. Those source/build checks do not prove deployed QML import selection: after an authorized switch and Caelestia restart, inspect the loaded `libcaelestia-configplugin.so` path with the old plugin absent, then manually lock for 60 seconds and verify DPMS-off plus physical-input wake while `WlSessionLock` remains locked.
...
For a long Q8 KV cache on a fixed-VRAM GPU, choose GPU layer policy per quantization profile. First prove whether the full profile fits under live desktop load. If it does, force `--n-gpu-layers all` and verify a long generation reaches high GPU SM with low CPU use; Axiom Q4 256K is the current example. If it does not, default `--fit` can preserve the cache but will create a CPU/GPU hybrid path; treat that as a throughput trade-off rather than a successful full-GPU deployment. A short response budget can be consumed entirely by reasoning; use a template-supported low reasoning setting when validating visible completion output.
