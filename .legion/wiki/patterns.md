...
Systemd-owned Hyprland helper services need explicit PATH coverage for commands embedded in checked-in configs. `hypridle.service` should be able to resolve `hyprctl`, `hyprlock`, `pidof`, `systemctl`, and `loginctl` from Nix store paths; do not rely on an interactive shell PATH for idle lock, DPMS, or suspend commands.
