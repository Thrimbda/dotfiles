...
For Axiom Keep Awake changes, prefer Caelestia's native `idleInhibitor` over custom no-sleep wrappers when the requirement is graphical-session idle inhibition. When the requirement is the stronger default "never sleep" state, keep the Caelestia UI state enabled but add or preserve a session-scoped login1 sleep inhibitor such as `axiom-caelestia-never-sleep.service` under `hyprland-session.target`; do not restore the old `axiom-sleep-mode` state machine unless a task explicitly asks for an allow-sleep toggle. Hypridle should remain scoped to idle lock and DPMS for the current Axiom default: validate the checked-in config has the 5 minute lock listener, the 10 minute DPMS off/on listener, and no `$suspend_cmd`, `systemctl suspend`, `loginctl suspend`, or `timeout = 900` automatic suspend listener. Validate the `07-caelestia-keep-awake` startup hook runs after the session-owned `06-caelestia-shell` runner, absence of stale `axiom-sleep-mode` packages/launchers/direct Hypridle overrides, and README documentation of the graphical-session boundary. Also validate generated helper text: Caelestia IPC helpers should call the evaluated `caelestia-shell` binary path or otherwise own command lookup, retry long enough for cold-start IPC registration, and launch asynchronously so desktop startup is not blocked. For never-sleep enforcement, validate the user service is wanted by and `PartOf=` `hyprland-session.target`, restarts on unexpected exit, and its generated script contains `systemd-inhibit --what=sleep --mode=block` with a long-running child. Avoid triggering live suspend or hibernate in tool sessions; record those as post-deploy Hyprland smoke checks.
...
For Caelestia launcher favourites, prefer Quickshell `DesktopEntry.id` values over duplicate desktop files. For top-level desktop files, Quickshell derives the id from the complete basename, so `share/applications/bytedance-feishu.desktop` is favourited as `bytedance-feishu`, not `bytedance-feishu.desktop`. Declare the desired default in `modules.desktop.caelestia.settings.launcher.favouriteApps`; when an existing mutable `shell.json` must be updated, use a narrow `modules.desktop.caelestia.session.preStart` migration that appends or corrects only the target id and leaves other user settings untouched. Because favourites are sorted entries, not discovery inputs, validate that `modules.desktop.caelestia.session.xdgDataDirs` exposes the package `share` path that contains the desktop file before diagnosing deeper launcher bugs. Validate the favourite setting, pre-start hook ordering, migration script syntax, package presence, desktop-entry existence, session runner XDG data dirs, and Axiom toplevel; live menu visibility still needs a real-session smoke check.
...
Systemd-owned Hyprland helper services need explicit PATH coverage for commands embedded in checked-in configs. `hypridle.service` should be able to resolve `hyprctl`, `hyprlock`, and `pidof` from Nix store paths for the current lock/DPMS-only idle policy; do not rely on an interactive shell PATH for idle lock or DPMS commands. Only require `systemctl` or `loginctl` there if a future task explicitly reintroduces an automatic suspend command.
