...
For Axiom Keep Awake changes, prefer Caelestia's native `idleInhibitor` over custom no-sleep wrappers when the requirement is graphical-session idle inhibition. Validate the `axiom-caelestia-keep-awake` user service, its `hyprland-session.target` and `caelestia-shell.service` wiring, absence of stale `axiom-sleep-mode` services/packages/launchers/direct Hypridle overrides, and README documentation of the graphical-session boundary. Also validate the generated helper text: if the helper uses Caelestia IPC, prefer the evaluated `caelestia-shell` binary path or explicit service PATH ownership rather than a Python CLI path that later resolves `caelestia-shell` from ambient `PATH`. Avoid triggering live suspend or hibernate in tool sessions; record those as post-deploy Hyprland smoke checks.
...
For Caelestia launcher favourites, prefer upstream desktop entry ids such as `bytedance-feishu.desktop` over duplicate desktop files. Declare the desired default in `modules.desktop.caelestia.settings.launcher.favouriteApps`; when an existing mutable `shell.json` must be updated, use a narrow user-service pre-start migration that appends only the missing id and leaves other user settings untouched. Because favourites are sorted entries, not discovery inputs, validate that `caelestia-shell.service.environment.XDG_DATA_DIRS` exposes the package `share` path that contains the desktop file before diagnosing deeper launcher bugs. Validate the favourite setting, pre-start hook ordering, migration script syntax, package presence, desktop-entry existence, service XDG data dirs, and Axiom toplevel; live menu visibility still needs a real-session smoke check.
