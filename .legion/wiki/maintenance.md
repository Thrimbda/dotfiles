...
- After deploying `auth-mini-acorn-gateway`, create or verify DNS-only records for `auth.0xc1.wang` and `auth-gateway.0xc1.wang`, switch Acorn, confirm `auth-mini.service` and all `auth-mini-gateway-*` services are active, confirm ACME certs for the new hostnames, bootstrap auth-mini issuer/RP config, and browser-smoke unauthenticated redirects, allowlisted access, denied-user `403`, logout, and Opencode WebSocket behavior.
