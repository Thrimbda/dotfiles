...
- After deploying `axiomctl-cli-consolidation`, run `axiomctl status`, switch from an SSH session with `axiomctl mode cli`, confirm the SSH session and remote access services survive, confirm a physically attached display reaches a TTY login, then switch back with `axiomctl mode desktop`. Run `axiomctl reload` from the graphical session to confirm it reaches the existing reload hooks. Compare `nvidia-smi --query-gpu=power.draw,pstate,display_active --format=csv` before and after CLI mode before making power-saving claims.
