...
- After deploying `axiom-autossh-c1-runtime-fix`, confirm `systemctl cat autossh-reverse-ssh.service` shows `c1@8.159.128.125`, a service-specific `GlobalKnownHostsFile`, and `UserKnownHostsFile=/dev/null`, then restart `autossh-reverse-ssh.service`, run `autossh-reverse-ssh-healthcheck.service`, and confirm remote `127.0.0.1:2223` exposes Axiom's local SSH host key.
