...
- After deploying `auth-mini-acorn-empty-response`, confirm `https://auth.0xc1.wang/` returns `302 Location: /web/` and `https://auth.0xc1.wang/web/` returns the auth-mini UI. The DNS-only Cloudflare records for `auth.0xc1.wang` and `auth-gateway.0xc1.wang` have been created; if a client still sees `198.18.x.x`, refresh its browser/proxy DNS cache. Then continue the original auth-mini rollout: confirm all `auth-mini-gateway-*` services are active, bootstrap auth-mini issuer/RP/admin state, and browser-smoke unauthenticated redirects, allowlisted access, denied-user `403`, logout, and Opencode WebSocket behavior.
