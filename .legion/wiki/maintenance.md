...
- The `auth-mini-gateway-login-failure` exact-identity authorization fix remains current, but its four-Acorn-instance restart note is superseded. After deploying `auth-mini-node-gateway-migration`, confirm Acorn retains auth-mini plus the `auth-gateway` and `frps-acorn` gateways, Axiom runs the status/OpenCode gateways before their loopback apps, and FRP targets `7779/7780`. In a fresh browser context, verify per-origin unauthenticated redirects, allowlisted Email OTP access to status/opencode/frps, OpenCode WebSocket/upload/streaming behavior, opaque host-only cookies, non-allowlisted denial, and logout without recording credentials or session material; expect existing status/OpenCode sessions to reset.
