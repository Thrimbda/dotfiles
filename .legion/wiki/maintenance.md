...
- After deploying `auth-mini-acorn-empty-response`, confirm `https://auth.0xc1.wang/` returns `302 Location: /web/` and `https://auth.0xc1.wang/web/` returns the auth-mini UI. The DNS-only Cloudflare records for `auth.0xc1.wang` and `auth-gateway.0xc1.wang` have been created; if a client still sees `198.18.x.x`, refresh its browser/proxy DNS cache. Bootstrap auth-mini issuer/RP/admin state as needed, but use `auth-mini-node-gateway-migration` rather than the original four-Acorn-instance rollout for current gateway placement.
...
- After deploying `hey-jpm-activation-safe-rebuild`, run `hey path home` and `hey hook startup` in the live Axiom session. Confirm the first successful activation rebuilds `~/.local/share/janet/jpm_tree/lib/spork`, `systemctl --user is-active hyprland-session.target` is active, and `pgrep -a caelestia` shows the session runner/shell.
...
- The Hypridle-listener portions of the `axiom-caelestia-idle-timeouts`, Caelestia WlSessionLock, `axiom-remove-never-sleep`, and `axiom-remove-idle-suspend` follow-ups are superseded by `axiom-hyprland-xwayland-idle-crash-fix`. Current Axiom verification must confirm Caelestia alone owns the 900 second lock and 1800 second DPMS actions, `hypridle.service` is absent, and no automatic sleep action exists. Do not use automated live suspend or hibernate tests as proof.
- After deploying the Caelestia WlSessionLock switch, start or restart the Hyprland/Caelestia session, run `caelestia shell lock lock`, unlock successfully, then confirm `SUPER+SHIFT+L` uses the same Caelestia lock surface. Also confirm no `hyprlock` binary or PAM service is present in the generated Axiom system closure.
- After deploying `axiom-remove-never-sleep`, start a new Hyprland/Caelestia session, confirm `caelestia shell idleInhibitor isEnabled` is a manual state rather than a forced startup default, confirm Caelestia owns the 15 minute lock and 30 minute DPMS actions, and confirm no repository-owned sleep-inhibitor user service is active. Do not use an automated live suspend test as proof.
- After deploying `axiom-remove-idle-suspend`, confirm Caelestia's active idle timeouts contain only lock and DPMS actions, `hypridle.service` is absent on Axiom, and no automatic suspend action exists. Do not use an automated live suspend test as proof.
...
- `axiom-lock-dpms-plugin-fix` passed split source-patch, configured-package build, plugin-`qmltypes`, exact build-input, `passthru.plugin`, and shell-closure checks. Axiom deployment and runtime import selection remain blocked because `sudo -n true` requires sudo authentication; no switch, Caelestia restart, or live lock test was attempted. After an authorized deployment and restart, prove the loaded `libcaelestia-configplugin.so` is under the patched plugin output with the original plugin absent and the active value still `60`. Then run the existing direct Caelestia IPC, 900-second idle, and `loginctl lock-session` paths, including early unlock and rapid unlock/relock, and manually verify DPMS turns off after 60 seconds and physical input wakes it while `WlSessionLock` remains locked. Confirm no extra idle daemon or automatic suspend/hibernate is active.
...
- After deploying `axiom-critical-network-resilience`, confirm `systemctl show autossh-reverse-ssh.service cloudflared.service clash-verge.service sshd.service` reports the intended OOM/resource settings, `systemctl list-timers '*healthcheck*'` shows the remaining local healthcheck timers, `curl --fail http://127.0.0.1:20241/ready` passes, `c1ctl autossh check` confirms the autossh remote endpoint key still matches `axiom`'s local host key, and `systemctl --user show 'app-clash\x2dverge@autostart.service'` reports the GUI drop-in. Then run each remaining timer-backed healthcheck service once through systemd and inspect journal output before relying on restart automation.
- After deploying `axiom-autossh-c1-runtime-fix` plus `axiom-autossh-c1ctl-check`, confirm `systemctl cat autossh-reverse-ssh.service` shows `c1@8.159.128.125`, a service-specific `GlobalKnownHostsFile`, and `UserKnownHostsFile=/dev/null`, restart `autossh-reverse-ssh.service`, and run `c1ctl autossh check` to confirm remote `127.0.0.1:2223` exposes Axiom's local SSH host key.
