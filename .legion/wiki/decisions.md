...
Axiom desktop power policy now defaults to Caelestia Keep Awake plus a graphical-session sleep inhibitor. Keep Caelestia's own Keep Awake UI as the visible source of truth by enabling Caelestia `idleInhibitor` after the session-owned runner starts, and enforce the stronger never-sleep default with `axiom-caelestia-never-sleep.service`, a user service tied to `hyprland-session.target` that runs `systemd-inhibit --what=sleep --mode=block`. The checked-in Hypridle idle policy should only lock at 5 minutes and turn DPMS off/on at 10 minutes; it must not contain a 15 minute automatic suspend listener unless a future scoped task explicitly restores automatic idle suspend. Do not restore the custom `axiom-sleep-mode` wrapper, Power Mode launcher entries, or Axiom-only Hypridle override unless a future task explicitly requires a user-facing allow-sleep toggle or pre-login/headless no-sleep policy. This policy is graphical-session scoped and does not grant logind `ignore-inhibit` or widen polkit power actions. Startup helpers for this policy should call the evaluated `caelestia-shell` binary directly, or otherwise own their command lookup, because the Caelestia Python CLI shells out to `caelestia-shell` by name. Because `caelestia-session start` backgrounds the shell runner, Keep Awake enablement must allow for cold-start IPC registration latency without blocking the rest of the startup hook chain.
