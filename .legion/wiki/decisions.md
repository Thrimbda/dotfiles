...
Axiom desktop power policy now keeps Caelestia's own Keep Awake UI as the visible source of truth by enabling Caelestia `idleInhibitor` after the session-owned runner starts, while Hypridle owns the idle lock and display power policy. The checked-in Hypridle idle policy locks at 15 minutes and turns DPMS off/on at 30 minutes; it must not contain an automatic suspend listener unless a future scoped task explicitly restores automatic idle suspend. Do not restore the custom `axiom-sleep-mode` wrapper, Power Mode launcher entries, Axiom-only Hypridle override, or a repository-owned sleep-inhibitor service unless a future task explicitly requires a user-facing allow-sleep toggle, default never-sleep behavior, or pre-login/headless no-sleep policy. This policy is graphical-session scoped and does not grant logind `ignore-inhibit` or widen polkit power actions. Startup helpers for this policy should call the evaluated `caelestia-shell` binary directly, or otherwise own their command lookup, because the Caelestia Python CLI shells out to `caelestia-shell` by name. Because `caelestia-session start` backgrounds the shell runner, Keep Awake enablement must allow for cold-start IPC registration latency without blocking the rest of the startup hook chain.
