...
`general.idle.lockDpmsTimeout` is a separate Axiom lock-scoped display policy supplied by split patches to the pinned Caelestia source: the Config C++ patch applies to the separately built `Caelestia.Config` plugin, while the QML timer patch applies to the main shell. A main shell `overrideAttrs` does not patch or rebuild that plugin automatically. Capture the unmodified plugin before either override, replace its exact shell `buildInputs` occurrence by resolved store path with the patched plugin, assert exactly one match, and extend `passthru.plugin` to the same patched output. Its typed default is `0`, while Axiom sets `60` through both declarative and mutable settings. It observes actual `WlSessionLock.locked`, permits DPMS-off only for the active per-lock epoch and timer identity, and turns DPMS on from input without unlocking. It neither replaces nor resets the independent 900/1800-second policy, enables the mechanism on other hosts, or introduces another daemon, suspend, or hibernate path.
