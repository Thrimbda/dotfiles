...
`general.idle.lockDpmsTimeout` is a separate Axiom lock-scoped display policy supplied by a local patch to the pinned Caelestia source. Its typed default is `0`, while Axiom sets `60` through both declarative and mutable settings. It observes actual `WlSessionLock.locked`, permits DPMS-off only for the active per-lock epoch and timer identity, and turns DPMS on from input without unlocking. It neither replaces nor resets the independent 900/1800-second policy, enables the mechanism on other hosts, or introduces another daemon, suspend, or hibernate path.
