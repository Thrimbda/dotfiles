...
Axiom desktop power policy treats Caelestia's Keep Awake UI as a manual `idleInhibitor` toggle, not as a default startup state. Caelestia is Axiom's sole automatic idle owner: its `general.idle.timeouts` retain the 900 second WlSessionLock action and 1800 second DPMS off/on actions, while the Axiom Hypridle user unit is disabled to prevent competing callbacks. Hypridle remains enabled by default for other hosts. Neither Hypridle nor Caelestia should contain a 600 second automatic sleep, suspend, hibernate, or `suspend-then-hibernate` idle action unless a future scoped task explicitly restores automatic idle suspend. Do not restore default `idleInhibitor enable` startup wiring, `hyprlock`, the custom `axiom-sleep-mode` wrapper, Power Mode launchers, Axiom-only Hypridle overrides, or a repository-owned sleep-inhibitor service unless a future task explicitly requires a separate lock client, a user-facing allow-sleep toggle, default never-sleep behavior, or pre-login/headless no-sleep policy. This policy is graphical-session scoped and does not grant logind `ignore-inhibit` or widen polkit power actions. Startup and idle helpers for this policy should call the evaluated `caelestia`/`caelestia-shell` binaries directly, or otherwise own their command lookup, because the Caelestia Python CLI shells out to `caelestia-shell` by name.

`general.idle.lockDpmsTimeout` is a separate Axiom lock-scoped display policy supplied by split patches to the pinned Caelestia source: the Config C++ patch applies to the separately built `Caelestia.Config` plugin, while the QML timer patch applies to the main shell. A main shell `overrideAttrs` does not patch or rebuild that plugin automatically. Capture the unmodified plugin before either override, replace its exact shell `buildInputs` occurrence by resolved store path with the patched plugin, assert exactly one match, and extend `passthru.plugin` to the same patched output. Its typed default is `0`, while Axiom sets `60` through both declarative and mutable settings. On Quickshell 0.3, arm the timer from `WlSessionLock.secureChanged`, not `lockedChanged`, because only the former reliably follows compositor-confirmed lock acquisition; keep `lockedChanged` for unlock cleanup. The timer permits DPMS-off only for the active per-lock epoch and timer identity. Axiom's Hyprland `misc.key_press_enables_dpms` and `misc.mouse_move_enables_dpms` own physical-input wake while retaining the lock; this host-wide behavior also applies to the 1800-second fallback and is user-approved. It neither resets the independent 900/1800-second policy, enables the mechanism on other hosts, nor introduces another daemon, suspend, or hibernate path.
...
For `pi-axiom.0xc1.wang`, `axiom` owns frp proxy `axiom-pi-web-http` from the local PI gateway `127.0.0.1:7782` to remote TCP `18082` on `acorn`; the gateway then proxies to PI WEB on `127.0.0.1:8504`. FRP must target the gateway rather than PI WEB. Shared FRPS makes `18082` wildcard-bound, but it is an nginx backend only and must remain absent from the NixOS firewall and Aliyun security group.
...
`auth-mini-gateway` protects the human-facing `0xc1.wang` routes, but enforcement is node-local. Acorn keeps the local `auth-gateway.0xc1.wang` and `frps-acorn.0xc1.wang` gateway instances. Axiom owns the proxy-mode gateway instances for `status-axiom.0xc1.wang`, `opencode-axiom.0xc1.wang`, and `pi-axiom.0xc1.wang` before their loopback applications; the corresponding Acorn vhosts terminate TLS and forward application traffic to FRP remote ports `18080/18081/18082` without a second `auth_request` boundary.

Auth-mini is the sole authority for authentication-method selection. The gateway verifies the auth-mini audience-bound token/session and grants upstream access only when the user ID exactly matches `ALLOW_USER_IDS`; unknown identities are denied. Email is not an authorization input for current downstream JWTs, and nonempty `ALLOW_EMAILS` fails gateway startup. Do not reintroduce gateway-owned Passkey/`amr` policy without a new security design.

Current upstream gateway behavior is same-origin: it validates returns against one `GATEWAY_PUBLIC_BASE_URL` and emits host-only cookies. Run one gateway instance per protected hostname instead of one central cross-host callback service. The current loopback allocation is Acorn `auth-gateway.0xc1.wang -> 7778`, Axiom `status-axiom.0xc1.wang -> 7779 -> Gatus 8080`, Axiom `opencode-axiom.0xc1.wang -> 7780 -> OpenCode 4096`, Acorn `frps-acorn.0xc1.wang -> 7781`, and Axiom `pi-axiom.0xc1.wang -> 7782 -> PI WEB 8504`.

Because authenticated PI WebSockets carry `c1` terminal and agent authority, Acorn nginx must accept upgrades only for the exact semantic Origin `https://pi-axiom.0xc1.wang`; sibling, foreign, suffixed, and missing Origin upgrades fail closed before FRP or Auth Mini. Ordinary PI HTTP/API traffic and unrelated vhosts remain outside this guard.
...
Gateway cookie secrets and allowlists are host-local agenix state: Acorn owns the environment for its retained gateways, and Axiom owns an independently encrypted environment for status/OpenCode/PI. Do not put gateway secrets, refresh tokens, plaintext allowlists, or supplied user IDs into Nix store paths, task docs, PR bodies, or logs. Backend ports `7777` through `7782`, application ports `8080/4096/8504`, and FRP remote ports `18080/18081/18082` must remain absent from public firewalls and the Aliyun security group.

Current audience-bound gateway rollout requires an exact JWT `aud` matching each gateway's normalized public hostname and user-ID-only authorization from signed `sub`. Axiom cleartext proxy gateways must set `UPSTREAM_PROTOCOL=http1`; Acorn deployments must continue to use the mandated Axiom `--build-host localhost` path.
