...
Axiom SSH-only operation uses `axiom-mode cli`, which persists and isolates `axiom-cli.target`. That target keeps `multi-user.target` as the service base, wants `getty@tty1.service` for local-console fallback, and conflicts with `graphical.target` so greetd/Hyprland stop. Return to desktop mode with `axiom-mode desktop`, which persists and isolates `graphical.target`. Keep this command independent of `hey`; it is a standalone Rust package under `packages/axiom-mode` with fixed target names and fixed privileged `systemctl` argv.
