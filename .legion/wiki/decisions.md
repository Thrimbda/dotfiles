...
Axiom SSH-only operation uses `axiomctl mode cli`, which persists and isolates `axiom-cli.target`. That target keeps `multi-user.target` as the service base, wants `getty@tty1.service` for local-console fallback, and conflicts with `graphical.target` so greetd/Hyprland stop. Return to desktop mode with `axiomctl mode desktop`, which persists and isolates `graphical.target`. Keep this host-control CLI as a standalone Rust package under `packages/axiomctl` with fixed target names and fixed privileged `systemctl` argv. Broad dotfiles/Nix workflows remain owned by `hey`; `axiomctl reload` is only a narrow fixed-argv bridge to the reviewed `hey reload` hook path.
