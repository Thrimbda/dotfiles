  ...
  modules = {
    ...
    desktop = {
      hyprland = rec {
        ...
        idle.time = 300;
        extraConfig = ''
          hl.workspace_rule({ workspace = "special:term", gaps_in = 3, gaps_out = 100, on_created_empty = "hey .scratch term" })
          hl.workspace_rule({ workspace = "special:pad", gaps_in = 3, gaps_out = { top = 40, right = 80, bottom = 40, left = 80 } })

          -- trigger when the lid is up
          hl.bind("switch:off:Lid Switch", hl.dsp.exec_cmd("hyprctl dispatch dpms on"), { locked = true })
          -- trigger when the lid is down
          hl.bind("switch:on:Lid Switch", hl.dsp.exec_cmd("hyprctl dispatch dpms off && hey .lock --no-fade-in --no-fade-out"), { locked = true })
        '';
      };
